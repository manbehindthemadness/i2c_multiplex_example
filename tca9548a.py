...
class TCA9548A_Channel:
    ...
    def __init__(self, tca, channel, debug=False):
        self.tca = tca
    ...
    def readfrom_into(self, address, buffer, **kwargs):
        ...
        self._debug('read', address, buffer)
        return result
    ...
    def writeto(self, address, buffer, **kwargs):
        ...
        if address == self.tca.address:
            raise ValueError("Device address must be different than TCA9548A address.")
        self._debug('write', address, buffer)
    ...
    def writeto_then_readfrom(self, address, buffer_out, buffer_in, **kwargs):
        ...
        if address == self.tca.address:
            raise ValueError("Device address must be different than TCA9548A address.")
        self._debug('write-read', address, buffer_out, buffer_in)
...
class TCA9548A:
    ...
    def __init__(self, i2c, address=_DEFAULT_ADDRESS, debug=False):
        self.i2c = i2c
    ...
    def __getitem__(self, key):
        ...
        if self.channels[key] is None:
            self.channels[key] = TCA9548A_Channel(self, key, debug=self.debug)
